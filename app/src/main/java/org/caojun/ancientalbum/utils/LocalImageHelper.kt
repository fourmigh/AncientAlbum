...
class LocalImageHelper private constructor(private val context: Context) {
    ...
    private fun initImage() {
        ...
        while (cursor.moveToNext()) {
            ...
            val file = File(path)
            //判断大图是否存在
            if (file.exists()) {//小图URI
//                var thumbUri = getThumbnail(id)
                //获取大图URI
                val originalUri = MediaStore.Images.Media.EXTERNAL_CONTENT_URI.buildUpon().appendPath(Integer.toString(id)).build().toString()
...
//                    thumbUri = originalUri
                ...
                val photoFile = Photo(/*thumbUri!!, */originalUri)
